	...
	$( document ).ready(
		function () {
						...
						function () {
							...
							selection.map(
								function (attachment) {
									...
									var font_tpl = $( "#blyd3d-font-tpl" ).val();
			...
			window.check_the_max_input_vars = function () {
				...
				var message_admin_notice = "<div class='blyd3d-error error'><p>" + msg + "</p></div>";
				if (all_champs_form >= kali_max_input_vars) {
					if ($( ".blyd3d-error" ).length) {
						$( ".blyd3d-error" ).html( "<p>" + msg + "</p>" );
					} else {
			...
			function get_tables_hierarchy(raw_tpl, element)
			{
			    ...
			    var row_selecter = element.siblings( "table.repeatable-fields-table" ).children( "tbody" ).children( ".kali-row-count" ).first();
			    var min_row = parseInt($(row_selecter).val());
				...
				if(new_key_index <= min_row) new_key_index = min_row + 1;
				var re = new RegExp( matches[0], 'g' );
				raw_tpl_tmp = raw_tpl_tmp.replace( re, new_key_index );
				$(row_selecter).val(new_key_index)
				return raw_tpl_tmp;
			}
							...
							function () {
								...
								selection.map(
									function (attachment) {
										...
										var isVideo = url_without_root.match(/\.(mp4|webm|ogg)$/i);
										if(isVideo) trigger.parent().find( ".media-preview" ).html( "<video width='40' height='40' src='" + attachment.url + "'></video>" );
										else trigger.parent().find( ".media-preview" ).html( "<img src='" + attachment.url + "'>" );
